> **This document is for reference only.** It documents system rules, architecture decisions, and process guidance. It does not control execution. All deployment execution is controlled by `pchk.md`.

# OrgPro System Specifications
...
All stacking contexts must use one of the five canonical layers below. **No new z-index value may be introduced outside these layers without updating this table first.**

| Layer | Tailwind class | z-index value | Purpose | Current components |
|-------|---------------|---------------|---------|-------------------|
| 1 — Chrome | `z-10` | 10 | Sticky app chrome that scrolls with the page (headers, toolbars) | `AppHeader` sticky header, `.mdx-sticky-toolbar` toolbar (`index.css`) |
| 2 — Grid | `z-20` | 20 | Sticky table rows/columns inside scrollable grids | `RSchedule` `<thead>` sticky row, `RSchedule` time-column `<td>` sticky left |
| 3 — Grid overlay | `z-30` | 30 | Elements that must paint above both sticky axes (corner cells, in-grid indicators) | `RSchedule` corner `<th>` (sticky left + top), `RSchedule` current-time line |
| 4 — Popover / Dialog | `z-50` | 50 | Modal overlays, dialogs, drawers, popovers, selects, context menus | `DialogOverlay`, `DialogContent`, `AlertDialogOverlay`, `AlertDialogContent`, `DrawerOverlay`, `DrawerContent`, `SelectContent`, `ContextMenuContent`, `ContextMenuSubContent` |
| 5 — System overlay | `z-[100]` | 100 | Full-screen celebration / blocking overlays that must sit above every dialog | `PomodoroTimer` celebration overlay |
